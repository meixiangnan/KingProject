Shader "myshader/rouhua"
{
	Properties
	{
		...
		_OriginAlphaThreshold("OriginAlphaThreshold", range(0.1, 1)) = 0.9	//原像素剔除的阈值
		[Toggle(_ShowOutline)] _ShowOutline("Show Outline", Int) = 1		//开启外轮廓的Toggle
		...
		SubShader
		{
			...
			Pass
			{
				...
				{
					//经过精度来划分一个圆，而后经过一个for循环来计算偏移，而后采样机上透明度。
					//我原本使用的精度的平方，可是这样unityshader会报一个错误，说迭代的次数太长，不能超过1024次，
					//可是实际上我并无用那么屡次，可是无法解决就手动输入精度
					float sum = 0;
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					//2D图片外轮廓
						...
						//2D图片的内发光
							...
							//这里获取到内发光的的透明度，而且作了渐变，让靠近边界的颜色更亮一些，原理的透明度的会愈来愈低
