...
public class UIPanel : UIRect
{
    ...
    void FillAllDrawCalls()
    {
        ...
        for (int i = 0; i < widgets.Count; ++i)
        {
            ...
            //下一个widget
            UIWidget nextWidget = null;
            if (i < widgets.Count - 1)
            {
                nextWidget = widgets[i + 1];
            }

            if (w.isFairyDrawCall)
            {
                //Debug.Log(w.transform.parent.name);


                Material mt = null;

                //if (w.GetComponent<MeshRenderer>()!=null&&w.GetComponent<MeshRenderer>().sharedMaterials.Length > 0)
                //{
                //    mt = w.GetComponent<MeshRenderer>().sharedMaterials[0];
                //}

                if (w.GetComponent<Renderer>() != null && w.GetComponent<Renderer>().sharedMaterials.Length > 0)
                {
                    mt = w.GetComponent<Renderer>().sharedMaterials[0];
                }

                if (nextWidget!=null&&nextWidget.isFairyDrawCall)
                {

                    //if(true)
                    if (!nextWidget.patchdraw)
                    {
                        UIDrawCall fairyDrawCall = UIDrawCall.Create(this, mt, mt.mainTexture, mt.shader);
                        w.drawCall = fairyDrawCall;
                        fairyDrawCall.depthStart = w.depth;
                        fairyDrawCall.depthEnd = fairyDrawCall.depthStart;
                        fairyDrawCall.panel = this;
                        fairyDrawCall.w = w;
                        fairyDrawCall.onRender = mOnRender;
                        fairyDrawCall.onSortingChange = w.onSortingChange;


                        drawCalls.Add(fairyDrawCall);
                    ...
                    if (mat != mt)
                    {
                        // Debug.Log("00=="+w.transform.parent.name);
                        //if(mt!=null)
                        //mt.name ="00"+ w.transform.parent.name;
                        Texture mttex=null;
                        Shader mtshader=null;

                        if (mt == null)
                        {
                            NGUISpine ns = w.GetComponent<NGUISpine>();
                            if (ns != null)
                            {
                              //  Debug.LogError(ns.animation.AnimationName+" notmat");
                            }
                        }
                        else
                        {
                            mttex = mt.mainTexture;
                            mtshader = mt.shader;
                        }

                        UIDrawCall fairyDrawCall = UIDrawCall.Create(this, mt, mttex, mtshader);
                        w.drawCall = fairyDrawCall;
                        fairyDrawCall.depthStart = w.depth;
                        fairyDrawCall.depthEnd = fairyDrawCall.depthStart;
                        fairyDrawCall.panel = this;
                ...
                {
                    // Debug.Log("11=="+w.transform.parent.name);
                    //if (mt != null)
                    //    mt.name ="11"+ w.transform.parent.name;
                    UIDrawCall fairyDrawCall = UIDrawCall.Create(this, mt,  null, null);
                    w.drawCall = fairyDrawCall;
                    fairyDrawCall.depthStart = w.depth;
                    fairyDrawCall.depthEnd = fairyDrawCall.depthStart;
                    fairyDrawCall.panel = this;
            ...
            if (w.isFairyDrawCall == false && nextWidget != null && nextWidget.isFairyDrawCall)
            {
                if (dc != null && dc.verts.Count != 0)
                {
                    drawCalls.Add(dc);
                    dc.UpdateGeometry(count,false);
                    dc.onRender = mOnRender;
                    mOnRender = null;
    ...
    void UpdateDrawCalls(int sortOrder)
    {
        ...
        for (int i = 0; i < drawCalls.Count; ++i)
        {
            ...
            //if (gameObject.name == "Build_1011-12,12")
            //{
            //    Debug.Log(i+"/"+drawCalls.Count+"=="+dc.mLastRenderQueue);
            //}
