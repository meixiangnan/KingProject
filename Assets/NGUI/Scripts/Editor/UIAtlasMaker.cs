...
public class UIAtlasMaker : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		if (atlas != null)
		{
            ...
            GUILayout.BeginHorizontal();
            {
                if (tex != null)
                {
                    if (GUILayout.Button("导出贴图", GUILayout.Width(120f)))
                    {
            ...
            GUILayout.BeginHorizontal();
            {
                if (tex != null)
                {
                    if (GUILayout.Button("导出什么图", GUILayout.Width(120f)))
                    {
                        string filePath = EditorUtility.SaveFolderPanel("保存贴图到指定文件夹", "", "");
                        ExportTexturePNGFromAtlas2(filePath, NGUISettings.atlas);
                    }
                }
            }
            GUILayout.EndHorizontal();
    ...
    static Color32[] suopix(int newWidth, int newHeight, Color32[] newPixels,int bili)
    {
        int targetWidth = newWidth / bili;
        int targetHeight = newHeight / bili;

        Color32[] newPixels0 = new Color32[targetWidth * targetHeight];

        int pianx = (newWidth - targetWidth) / 2;
        int piany = (newHeight - targetHeight) / 2;

        Debug.Log(newPixels0.Length + " " + targetWidth + " " + targetHeight);
        for (int y = 0; y < targetHeight; y++)
        {
            for (int x = 0; x < targetWidth; x++)
            {
                int newIndex = (newHeight - 1 - (y * bili)) * newWidth + (x * bili);
                int oldIndex = (targetHeight - 1 - y) * targetWidth + x;
            //    Debug.Log(newIndex + " " + oldIndex);
                newPixels0[oldIndex] = newPixels[newIndex];
            }
        }

        Color32[] newPixels1 = new Color32[newWidth * newHeight];

        for (int y = 0; y < targetHeight; y++)
        {
            for (int x = 0; x < targetWidth; x++)
            {
                int newIndex = (newHeight - 1 - (y + piany)) * newWidth + (x + pianx);
                int oldIndex = (targetHeight - 1 - y) * targetWidth + x;
                newPixels1[newIndex] = newPixels0[oldIndex];
            }
        }
    ...
    static void ExportTexturePNGFromAtlas27(string folderPath, INGUIAtlas atlas)
    {
        ...
        foreach (var es in exitSpritesList)
        {
            ...
            newPixels=suopix(newWidth, newHeight, newPixels,4);
    ...
    static void ExportTexturePNGFromAtlas2(string folderPath, INGUIAtlas atlas)
    {
        List<UISpriteData> exitSpritesList = atlas.spriteList;
        Texture2D atlasTexture = NGUIEditorTools.ImportTexture(atlas.texture, true, false, !atlas.premultipliedAlpha);
        int oldwith = atlasTexture.width;
        ...
        Color32[] newPixels0 = new Color32[oldwith * oldHeight];
        foreach (var es in exitSpritesList)
        {
            int xmin = Mathf.Clamp(es.x, 0, oldwith);
            int ymin = Mathf.Clamp(es.y, 0, oldHeight);
            ...
            newPixels= suopix(newWidth, newHeight, newPixels, 2);
        ...
        Texture2D t = new Texture2D(oldwith, oldHeight);
        t.SetPixels32(newPixels0);
        t.Apply();
        byte[] bytes = t.EncodeToPNG();
        Texture2D.DestroyImmediate(t);
        t = null;
        ...
        try
        {
            using (FileStream fs = new FileStream(folderPath + "/" + "alluti_hu" + ".png", FileMode.CreateNew))
            {
                BinaryWriter writer = new BinaryWriter(fs);
                writer.Write(bytes);
            }
    ...
    static void ExportTexturePNGFromAtlas(string folderPath, INGUIAtlas atlas)
    {
        List<UISpriteData> exitSpritesList = atlas.spriteList;
        Texture2D atlasTexture = NGUIEditorTools.ImportTexture(atlas.texture, true, false, !atlas.premultipliedAlpha);
        int oldwith = atlasTexture.width;
