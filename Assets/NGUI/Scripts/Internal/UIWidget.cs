...
{
    //是否可以点击穿透
    public bool isTouchContinue;
    public void setTouchContinue(bool flag)
    {
        this.isTouchContinue = flag;
    }

    public bool isFairyDrawCall = false;
    public bool patchdraw = true;
    //sort改变的时候被通知
    public virtual void onSortingChange(int value)
    {
        Debug.Log("未实现的 sorting change");
    }
    ...
    static public int PanelCompareFunc(UIWidget left, UIWidget right)
    {
        ...
        if (left.isFairyDrawCall && !right.isFairyDrawCall) return -1;
        if (!left.isFairyDrawCall && right.isFairyDrawCall) return 1;
