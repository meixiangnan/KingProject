...
public class UIScrollView : MonoBehaviour
{
    ...
    private Vector3 pressInputPos;

    private Vector3 releasePos;

    private Vector3 tempNormalPos = Vector3.zero;

    private Vector2 tempClipOffset;
    private long pressUpTime = 0;
    private long pressDownTime = 0;
    private long lastDragTime = 0;
    private float speed = 0;
    private bool isStartCat = false;
    public void MoveAbsoluteCatStudio(Vector3 absolute)
    {
        ...
        mTrans.localPosition = pressPos + relative;
        Vector2 co = mPanel.clipOffset;
        co.x = tempClipOffset.x - relative.x;
        co.y = tempClipOffset.y - relative.y;
    ...
    public void MoveListToLocalPosition(Vector3 relative)
    {
        mTrans.localPosition = mTrans.localPosition + relative;
        Vector2 co = mPanel.clipOffset;
        co.x -= relative.x;
        co.y -= relative.y;
        mPanel.clipOffset = co;
    ...
    private Vector3 SpringDampen(ref Vector3 velocity, float strength, float deltaTime)
    {
    ...
    void OnEnable ()
	{
        ...
        if (dragEffect == DragEffect.CATSTUDIO)
        {
            speed = 0;
        }
        list.Add(this);
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			if (pressed)
			{
                ...
                if (dragEffect == DragEffect.CATSTUDIO)
                {
                    pressWorldPos = new Vector3(mLastPos.x, mLastPos.y, mLastPos.z);

                    pressPos = new Vector3(mTrans.localPosition.x, mTrans.localPosition.y, mTrans.localPosition.z);

                    pressInputPos = Input.mousePosition;

                    tempClipOffset = new Vector2(mPanel.clipOffset.x, mPanel.clipOffset.y);
                    pressDownTime = UTools.currentTimeMillis();
                    speed = 0;
                    isStartCat = true;
                    tempNormalPos = Vector3.zero;
                }
			...
			else
			{
                if (dragEffect == DragEffect.CATSTUDIO)
                {
                    ...
                    //releasePos = new Vector3(mTrans.localPosition.x, mTrans.localPosition.y, mTrans.localPosition.z);

                    releasePos = /*UICamera.currentCamera.ScreenToWorldPoint(*/Input.mousePosition/*)*/;
                    ...
                    {
                        dis = releasePos.x - pressInputPos.x;
                    }
                    else if (movement == Movement.Vertical)
                    {
                        dis = releasePos.y - pressInputPos.y;
                    }
	...
	public void Drag ()
	{
		...
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
                ...
                if (dragEffect == DragEffect.CATSTUDIO)
                {
                    offset = currentPos - pressWorldPos;
                }
                ...
                if (dragEffect == DragEffect.CATSTUDIO)//后添加的模式
                {
                    lastDragTime = UTools.currentTimeMillis();

                    Bounds b = bounds;
                    Vector3 constraint = mPanel.CalculateConstrainOffset(b.min, b.max);
                    if (movement != Movement.Horizontal) constraint.x = 0f;
                    if (movement != Movement.Vertical) constraint.y = 0f;
                    ...
                    if (constraint.sqrMagnitude > 0.1f)
                    {
                        ...
                        if (movement == Movement.Horizontal)
                        {
                            scale = (float)(panel.GetViewSize().x / Screen.width);
                        }
                        else
                        {
                            scale = (float)(panel.GetViewSize().y / Screen.height);
                        }

                        MoveAbsoluteCatStudio(tempNormalPos + (offset - tempNormalPos) * 0.5f * scale);
                    }
                    else//正常滑动
                    {
                        MoveAbsoluteCatStudio(offset);
                        tempNormalPos = new Vector3(offset.x, offset.y, offset.z);
                    }
                }else
				...
				if (constrainOnDrag && restrictWithinPanel &&
                    ...
                    && dragEffect != DragEffect.CATSTUDIO)
				{
			...
			if (mMomentum.magnitude > 0.0001f || Mathf.Abs(mScroll) > 0.0001f)
			{
                ...
                if (dragEffect == DragEffect.CATSTUDIO)
                {
                    ...
                    if (speed != 0)
                    {
                        ...
                        if ((movement == Movement.Horizontal && constraint.x != 0) || (movement == Movement.Vertical && constraint.y != 0))
                        {
                            speed *= 0.6f;
                        }

                        if (movement == Movement.Horizontal && speed != 0)
                        {
                            MoveListToLocalPosition(new Vector3(speed, 0, 0));
                        }
                        else if (movement == Movement.Vertical && speed != 0)
                        {
                            MoveListToLocalPosition(new Vector3(0, speed, 0));
                        }
                    }
                    else
                    {
    ...
    public void RestrictCatStudioWithinBounds()
    {
        mCalculatedBounds = false;
        Bounds b = bounds;
